...
class UserAdmin(BaseUserAdmin):
    ...
    # Make primary key readonly in admin change view to avoid accidental edits
    readonly_fields = ("id", "created_at", "last_login", "password_changed_at")
    ...
    def role_badge(self, obj):
        ...
        if not obj.role:
            return format_html(
                '<span style="background-color: #6c757d; color: white; padding: 3px 7px; border-radius: 3px; font-size: 0.8em;">No Role</span>'
            )
    ...
    def status_badges(self, obj):
        ...
        if obj.is_approved:
            badges.append(
                '<span style="background-color: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: 0.75em; margin-right: 3px;">Approved</span>'
            )
        else:
            badges.append(
                '<span style="background-color: #ffc107; color: black; padding: 2px 6px; border-radius: 3px; font-size: 0.75em; margin-right: 3px;">Pending</span>'
            )

        if obj.is_verified:
            badges.append(
                '<span style="background-color: #007bff; color: white; padding: 2px 6px; border-radius: 3px; font-size: 0.75em;">Verified</span>'
            )
        else:
            badges.append(
                '<span style="background-color: #6c757d; color: white; padding: 2px 6px; border-radius: 3px; font-size: 0.75em;">Unverified</span>'
            )
    ...
    def security_status(self, obj):
        ...
        if obj.account_locked_until and obj.account_locked_until > timezone.now():
            indicators.append(
                '<span style="color: #dc3545; font-weight: bold;" title="Account Locked">🔒</span>'
            )
        elif obj.failed_login_attempts > 0:
            indicators.append(
                f'<span style="color: #ffc107;" title="{obj.failed_login_attempts} failed attempts">⚠️</span>'
            )

        if obj.must_change_password:
            indicators.append(
                '<span style="color: #007bff;" title="Password change required">🔑</span>'
            )
    ...
    def login_info(self, obj):
        ...
        if obj.last_login:
            ...
            if days_since == 0:
                ...
                return format_html(
                    '<span style="color: #ffc107;">{} days ago</span>', days_since
                )
            else:
                return format_html(
                    '<span style="color: #dc3545;">{} days ago</span>', days_since
                )
    ...
    def require_password_change(self, request, queryset):
        ...
        self.message_user(
            request,
            f"{updated} users will be required to change their password on next login.",
        )
    ...
    def enable_2fa(self, request, queryset):
        ...
        for user in queryset:
            try:
                ...
                self.message_user(
                    request,
                    f"Failed to enable 2FA for {user.email}: {str(e)}",
    ...
    def disable_2fa(self, request, queryset):
        ...
        for user in queryset:
            try:
                ...
                self.message_user(
                    request,
                    f"Failed to disable 2FA for {user.email}: {str(e)}",
                    level=messages.ERROR,
                )
...
class UserRoleAdmin(admin.ModelAdmin):
    ...
    def status_badge(self, obj):
        ...
        if obj.is_active:
            ...
            return format_html(
                '<span style="background-color: #dc3545; color: white; padding: 3px 7px; border-radius: 3px;">Inactive</span>'
            )
...
class LoginAttemptAdmin(admin.ModelAdmin):
    ...
    def failure_reason_display(self, obj):
        ...
        return format_html(
            ...
            color,
            obj.failure_reason.replace("_", " ").title(),
        )
    ...
    def user_link(self, obj):
        ...
        if obj.user:
            url = reverse("admin:accounts_user_change", args=[obj.user.id])
            # Prefer showing the user's full name when available, fall back to email
            display = obj.user.get_full_name() or obj.user.email
...
class UserSessionAdmin(admin.ModelAdmin):
    ...
    def status_badge(self, obj):
        ...
        if obj.revoked_at:
            return format_html(
                '<span style="background-color: #dc3545; color: white; padding: 3px 7px; border-radius: 3px;">Revoked</span>'
            )
        elif obj.is_expired():
            return format_html(
                '<span style="background-color: #6c757d; color: white; padding: 3px 7px; border-radius: 3px;">Expired</span>'
            )
        else:
            return format_html(
                '<span style="background-color: #28a745; color: white; padding: 3px 7px; border-radius: 3px;">Active</span>'
...
class UserRoleHistoryAdmin(admin.ModelAdmin):
    ...
    def reason_badge(self, obj):
        ...
        if not obj.reason:
            return format_html(
                '<span style="background-color: #6c757d; color: white; padding: 2px 6px; border-radius: 3px; font-size: 0.8em;">No Reason</span>'
            )
        ...
        return format_html(
            '<span style="background-color: {}; color: white; padding: 2px 6px; border-radius: 3px; font-size: 0.8em;">{}</span>',
            color,
            obj.reason[:20] + ("..." if len(obj.reason) > 20 else ""),
        )
