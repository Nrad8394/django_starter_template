        ...
        try:
            ...
            try:
                ...
                UserSession.objects.filter(id=session.id).update(
                    is_active=False, expires_at=timezone.now()
                )
                return
            ...
            if not session.device_info:
                ...
                session.device_info = DeviceDetectionService.parse_user_agent(
                    user_agent
                )
            ...
            if request.user.is_authenticated:
                try:
                    ...
                    if existing_session:
                        ...
                        try:
                            ...
                            existing_session.revoked_at = (
                                None  # Clear revocation if reactivating
                            )
                            existing_session.save(
                                update_fields=["expires_at", "revoked_at"]
                            )
                            django_session.save(update_fields=["expire_date"])
                        except Session.DoesNotExist:
                    ...
                    logger.error(
                        f"Failed to create/reactivate session record for user {request.user}: {e}"
                    )
...
class LoginSecurityMiddleware:
    ...
    def _extract_email_from_request(self, request):
        ...
        # JSON request body
        if not email and request.META.get("CONTENT_TYPE", "").startswith(
            "application/json"
        ):
            try:
                import json
    ...
    def _track_login_attempt(self, request, response):
        ...
        if is_admin_login:
            ...
            # API login: some API backends (jwt-based) do not perform a session
            # login (SESSION_LOGIN=False) so request.user may still be
            # Anonymous at this point. Detect success by checking the response
            # body for tokens or user data in addition to request.user.
            status_ok = response.status_code in [200, 201]
            resp_data = getattr(response, "data", {}) or {}

            # Consider successful if either session auth was set, or the
            # response contains an access token, refresh token, verified flag
            # or user payload.
            has_tokens = bool(resp_data.get("access") or resp_data.get("refresh"))
            has_user = bool(resp_data.get("user"))
            verified_flag = resp_data.get("verified") is True

            success = status_ok and (
                request.user.is_authenticated or has_tokens or has_user or verified_flag
            )
        else:
            # Fallback: authenticated user + success status
            success = request.user.is_authenticated and response.status_code in [
                200,
                201,
                302,
            ]
        ...
        try:
            # Find user if successful. Be careful: for JWT API logins
            # request.user may still be AnonymousUser even when the response
            # contains tokens. Only assign a User instance when authenticated
            # or when we can resolve it by email.
            user = None
            if success:
            ...
            if success:
                # Handle successful login (only if we resolved a real user)
                if user:
                ...
                logger.info(f"Login successful for {user_identifier} from {ip_address}")
            else:
                # Handle failed login
                if user:
                ...
                logger = logging.getLogger(__name__)
                logger.warning(f"Login failed for {email or 'unknown'} from {ip_address}")
            ...
            # When creating session fallback, don't assign AnonymousUser to FK
            session_defaults = {
                "ip_address": ip_address,
                "user_agent": user_agent,
            ...
            suspicious_sessions = UserSession.detect_suspicious_sessions(
            ...
            if suspicious_sessions:
                ...
                logger = logging.getLogger(__name__)
                user_ident = getattr(request.user, "email", email or "unknown")
                logger.warning(
                    f"Suspicious sessions detected for user {user_ident}: "
