...
class NotificationTemplateAdmin(admin.ModelAdmin):
    ...
    list_filter = ("template_type", "priority", "is_active", "created_at")
    ...
    fieldsets = (
        ...
        (_("Template Content"), {"fields": ("subject", "body", "variables")}),
        (_("Settings"), {"fields": ("priority",)}),
        (
            _("Timestamps"),
    ...
    def export_template_data(self, request, queryset):
        ...
        response["Content-Disposition"] = (
            'attachment; filename="notification_templates.csv"'
        )
    ...
    def bulk_update_priority(self, request, queryset):
        ...
        if "apply" in request.POST:
            ...
            self.message_user(
                request, f"Updated {updated} template(s) to priority '{priority}'."
            )
            return
        ...
        return render(
            request,
            "admin/bulk_update_priority.html",
            {
                "templates": queryset,
                "priority_choices": NotificationTemplate._meta.get_field(
                    "priority"
                ).choices,
            },
        )
...
class NotificationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_("Scheduling"), {"fields": ("scheduled_at", "sent_at", "delivered_at")}),
        (
            _("Related Objects"),
            {"fields": ("content_type", "object_id", "related_url")},
        ),
        (_("Error Handling"), {"fields": ("retry_count", "max_retries", "last_error")}),
        (
            _("Timestamps"),
            {"fields": ("created_at", "updated_at"), "classes": ("collapse",)},
        ),
    ...
    def template_link(self, obj):
        ...
        if obj.template:
            url = reverse(
                "admin:notifications_notificationtemplate_change",
    ...
    def delivery_status(self, obj):
        ...
        if delivered == total:
            return format_html(
                '<span style="color: green;">{}/{} ✓</span>', delivered, total
            )
        elif delivered > 0:
            return format_html(
                '<span style="color: orange;">{}/{} ⚠️</span>', delivered, total
            )
        else:
            return format_html(
                '<span style="color: red;">{}/{} ✗</span>', delivered, total
            )
        ...
        for notification in queryset.select_related("recipient", "template"):
            writer.writerow(
                [
                    ...
                    (
                        notification.scheduled_at.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def bulk_update_priority(self, request, queryset):
        ...
        if "apply" in request.POST:
            ...
            updated = queryset.update(priority=priority)
            self.message_user(
                request, f"Updated {updated} notification(s) to priority '{priority}'."
            )
            return
        ...
        return render(
            request,
            "admin/bulk_update_notification_priority.html",
            {
...
class NotificationDeliveryAdmin(admin.ModelAdmin):
    list_display = (
        ...
        "status_badge",
        "provider_display",
        "recipient_address_preview",
        "sent_at",
        "created_at",
    )
    list_filter = ("delivery_method", "status", "provider", "sent_at", "created_at")
    search_fields = (
    ...
    actions = ["mark_as_delivered", "retry_failed_deliveries", "export_delivery_data"]
    ...
    fieldsets = (
        ...
        (_("Timing"), {"fields": ("sent_at", "delivered_at")}),
        (_("Error Handling"), {"fields": ("error_message", "retry_count")}),
        (
            _("Timestamps"),
            {"fields": ("created_at", "updated_at"), "classes": ("collapse",)},
        ),
    ...
    def notification_link(self, obj):
        ...
        if obj.notification:
            ...
            return format_html(
                '<a href="{}">Notification #{}</a>', url, obj.notification.id
            )
        return "-"
    ...
    def provider_display(self, obj):
        ...
        if obj.provider:
            return format_html(
                '<span style="font-weight: bold;">{}</span>', obj.provider.title()
            )
        return "-"
    ...
    def recipient_address_preview(self, obj):
        ...
        if obj.recipient_address:
            if "@" in obj.recipient_address:  # Email
                ...
                return (
                    obj.recipient_address[:10] + "..."
                    if len(obj.recipient_address) > 10
    ...
    def export_delivery_data(self, request, queryset):
        ...
        response = HttpResponse(content_type="text/csv")
        response["Content-Disposition"] = (
            'attachment; filename="notification_deliveries.csv"'
        )
        ...
        for delivery in queryset.select_related("notification"):
            writer.writerow(
                [
                    ...
                    (
                        delivery.sent_at.strftime("%Y-%m-%d %H:%M:%S")
                        if delivery.sent_at
                        else ""
                    ),
                    (
                        delivery.delivered_at.strftime("%Y-%m-%d %H:%M:%S")
...
class NotificationPreferenceAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_("Quiet Hours"), {"fields": ("quiet_hours_start", "quiet_hours_end")}),
        (
    ...
    def category_summary(self, obj):
        ...
        if categories:
            return format_html(
                '<span style="font-size: 0.8em;">{}</span>', ", ".join(categories)
            )
    ...
    def quiet_hours_display(self, obj):
        ...
        return format_html(
            '<span style="color: gray; font-size: 0.8em;">Not set</span>'
        )
    ...
    def export_preferences_data(self, request, queryset):
        ...
        response = HttpResponse(content_type="text/csv")
        response["Content-Disposition"] = (
            'attachment; filename="notification_preferences.csv"'
        )
    ...
    def bulk_update_channels(self, request, queryset):
        ...
        return render(
            request, "admin/bulk_update_channels.html", {"preferences": queryset}
        )
...
class NotificationEventAdmin(admin.ModelAdmin):
    ...
    ordering = ("event_type",)
    readonly_fields = ("created_at", "updated_at")
    actions = ["activate_events", "deactivate_events", "export_event_data"]
    ...
    def export_event_data(self, request, queryset):
        ...
        response = HttpResponse(content_type="text/csv")
        response["Content-Disposition"] = (
            'attachment; filename="notification_events.csv"'
        )
        ...
        for event in queryset.select_related(
            ...
            writer.writerow(
                [
                    ...
                    event.is_active,
                    (
                        event.default_email_template.name
                        if event.default_email_template
                        else ""
                    ),
                    (
                        event.default_sms_template.name
                        if event.default_sms_template
                        else ""
                    ),
                    (
                        event.default_push_template.name
                        if event.default_push_template
                        else ""
                    ),
                    (
                        event.default_in_app_template.name
                        if event.default_in_app_template
                        else ""
                    ),
                ]
            )
