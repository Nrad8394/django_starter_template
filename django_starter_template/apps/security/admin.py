...
class AuditLogAdmin(admin.ModelAdmin):
    list_display = (
        ...
        "severity_badge",
        "ip_address_display",
        "request_path_preview",
        "response_status_badge",
    )
    list_filter = ("event_type", "severity", "timestamp", "user", "request_method")
    search_fields = (
        "user__email",
        "user__first_name",
        "user__last_name",
    ...
    ordering = ("-timestamp",)
    actions = ["export_audit_data", "filter_by_severity", "filter_security_events"]
    ...
    def filter_security_events(self, request, queryset):
        ...
        self.message_user(
            request, f"Showing {filtered.count()} security-related event(s)."
        )
...
class RateLimitAdmin(admin.ModelAdmin):
    ...
    list_filter = ("limit_type", "is_blocked", "endpoint", "window_start")
    search_fields = ("identifier", "endpoint")
    readonly_fields = ("id", "window_start", "window_end")
    ordering = ("-window_start",)
    actions = ["unblock_limits", "reset_counters", "export_rate_limit_data"]
    ...
    def limit_type_badge(self, obj):
        ...
        type_colors = {"ip": "blue", "user": "green", "api_key": "orange"}
        color = type_colors.get(obj.limit_type, "gray")
        return format_html(
            '<span style="background-color: {}; color: white; padding: 3px 8px; '
            'border-radius: 4px; font-size: 0.8em;">{}</span>',
            color,
            obj.get_limit_type_display(),
        )
    ...
    def window_status(self, obj):
        ...
        if now > obj.window_end:
            ...
            return format_html(
                '<span style="color: green;">{} min left</span>', minutes
            )
        ...
        for limit in queryset:
            writer.writerow(
                [
                    ...
                    limit.is_blocked,
                    (
                        limit.blocked_until.strftime("%Y-%m-%d %H:%M:%S")
...
class SecurityEventAdmin(admin.ModelAdmin):
    ...
    list_filter = ("event_type", "severity", "status", "created_at", "resolved_at")
    search_fields = (
        "title",
        "description",
        "user__email",
    ...
    def resolution_status(self, obj):
        ...
        if obj.status == "resolved":
            ...
            return format_html(
                '<span style="color: orange; font-size: 0.8em;">False Positive</span>'
            )
        elif obj.status == "ignored":
            return format_html(
                '<span style="color: gray; font-size: 0.8em;">Ignored</span>'
            )
        else:
            return format_html(
                '<span style="color: red; font-size: 0.8em;">Active</span>'
            )
    ...
    def mark_as_false_positive(self, request, queryset):
        ...
        updated = queryset.filter(status="active").update(
            status=SecurityEvent.Status.FALSE_POSITIVE
        )
        self.message_user(
            request,
            ngettext(
    ...
    mark_as_false_positive.short_description = _(
    ...
    def mark_as_ignored(self, request, queryset):
        ...
        updated = queryset.filter(status="active").update(
            status=SecurityEvent.Status.IGNORED
        )
        ...
        for event in queryset.select_related("user", "resolved_by"):
            writer.writerow(
                [
                    ...
                    (
                        event.resolved_at.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def bulk_resolve_with_note(self, request, queryset):
        ...
        return render(request, "admin/bulk_resolve_events.html", {"events": queryset})
...
class SecuritySettingsAdmin(admin.ModelAdmin):
    ...
    def config_summary(self, obj):
        ...
        if obj.config:
            ...
            return format_html(
                '<span style="font-size: 0.8em; color: #666;">{}</span>', summary
            )
        return format_html(
            '<span style="color: gray; font-size: 0.8em;">No config</span>'
        )
...
class APIKeyAdmin(admin.ModelAdmin):
    ...
    list_filter = ("key_type", "is_active", "created_at", "expires_at")
    search_fields = (
        "name",
        "user__email",
        "user__first_name",
    ...
    actions = [
        ...
        "regenerate_keys",
        "export_api_keys",
        "bulk_update_rate_limits",
    ]

    fieldsets = (
        (_("Basic Information"), {"fields": ("id", "name", "key_type", "key", "user")}),
        (
            _("Configuration"),
            {"fields": ("permissions", "rate_limit", "is_active", "expires_at")},
        ),
        (_("Usage"), {"fields": ("last_used_at",), "classes": ("collapse",)}),
        (
            _("Timestamps"),
            {"fields": ("created_at", "updated_at"), "classes": ("collapse",)},
        ),
    ...
    def key_type_badge(self, obj):
        ...
        type_colors = {"service": "blue", "user": "green", "integration": "orange"}
        color = type_colors.get(obj.key_type, "gray")
        return format_html(
            '<span style="background-color: {}; color: white; padding: 3px 8px; '
            'border-radius: 4px; font-size: 0.8em;">{}</span>',
            color,
            obj.get_key_type_display(),
        )
    ...
    def key_preview(self, obj):
        ...
        if obj.key:
            masked = (
                obj.key[:8] + "..." + obj.key[-4:] if len(obj.key) > 12 else obj.key
            )
    ...
    def last_used_display(self, obj):
        ...
        if obj.last_used_at:
            ...
            if days_since == 0:
                return format_html(
                    '<span style="color: green; font-size: 0.8em;">Today</span>'
                )
            elif days_since == 1:
                return format_html(
                    '<span style="color: blue; font-size: 0.8em;">Yesterday</span>'
                )
            elif days_since <= 7:
                return format_html(
                    '<span style="color: orange; font-size: 0.8em;">{} days ago</span>',
                    days_since,
                )
            else:
                return format_html(
                    '<span style="color: red; font-size: 0.8em;">{} days ago</span>',
                    days_since,
                )
        return format_html('<span style="color: gray; font-size: 0.8em;">Never</span>')
    ...
    def rate_limit_display(self, obj):
        ...
        return format_html(
            '<span style="font-size: 0.8em;">{}/hour</span>', obj.rate_limit
        )
    ...
    def bulk_update_rate_limits(self, request, queryset):
        ...
        if "apply" in request.POST:
            ...
            if rate_limit:
                ...
                self.message_user(
                    request, f"Updated rate limit for {updated} API key(s)."
                )
            return
        ...
        return render(
            request, "admin/bulk_update_rate_limits.html", {"api_keys": queryset}
        )
