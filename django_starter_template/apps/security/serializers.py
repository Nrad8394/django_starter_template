...
class SecurityEventSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        # Handle resolution
        if (
            validated_data.get("status") == SecurityEvent.Status.RESOLVED
...
class ResolveEventSerializer(serializers.Serializer):
    ...
    notes = serializers.CharField(
        required=False, allow_blank=True, help_text="Resolution notes"
    )
...
class LogSecurityEventSerializer(serializers.Serializer):
    ...
    severity = serializers.ChoiceField(
        choices=["low", "medium", "high", "critical"], help_text="Event severity level"
    )
    detection_data = serializers.DictField(
        required=False, help_text="Additional detection data"
    )
